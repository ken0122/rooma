...
import { constrainPositionToMount, defaultMountedPosition, mountLockedAxes, MOUNT_SURFACE_LABELS, nearestWallFace, type MountSurface, type WallFace } from "@/lib/engine/mounting";
...
type EditorMetrics = SpatialMetrics & { position: Point3; rotationY: number; limits?: { min: Point3; max: Point3 }; issues: string[]; mount: { surface: MountSurface; label: string; lockedAxes: Axis3[] } };
...
function MetricInput({ value, unit = "m", min, max, step = 0.05, ariaLabel, disabled = false, onCommit }: { value: number; unit?: string; min?: number; max?: number; step?: number; ariaLabel: string; disabled?: boolean; onCommit: (value: number) => void }) {
  ...
  return <label className={`metric-input ${disabled ? "locked" : ""}`} title={disabled ? "该坐标由素材吸附面锁定" : undefined}><input aria-label={ariaLabel} disabled={disabled} inputMode="decimal" min={min} max={max} step={step} type="number" value={draft} onBlur={commit} onChange={event => setDraft(event.target.value)} onKeyDown={event => {
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const buildParametric = (group: THREE.Group, kind: string, requestedSize: Point3) => {
      ...
      if (kind === "door") {
        ...
        cylinder(radius, h * .34, 0, h * .17, 0, group, true);
        cylinder(radius * .82, h * .66, 0, h * .67, 0, group);
      }
    ...
    const finishObject = (group: THREE.Group, kind: string, label: string, position: [number, number, number], requestedSize?: Point3, requestedId?: string) => {
      ...
      const asset = getParametricAsset(kind);
      allocatedIds.add(id);
      group.userData = { id, kind, label, selectable: true, mountSurface: asset?.defaultMount.surface ?? "floor" };
      buildParametric(group, kind, requestedSize ?? asset?.defaultSize ?? { x: 1, y: 1, z: 1 });
      group.traverse(child => { child.userData.root = group; }); addPickProxy(group); group.position.set(...position);
      if (asset?.defaultMount.surface === "wall") group.userData.wallFace = nearestWallFace(group.position, group.userData.parametricSize as Point3, 0, room);
      selectable.add(group); layout.add(group); updateBounds(group); return group;
    };
    ...
    const mountInfoFor = (object: THREE.Group) => {
      const asset = getParametricAsset(object.userData.kind as string);
      const surface = asset?.defaultMount.surface ?? "floor";
      const size = object.userData.parametricSize as Point3;
      const wallFace = surface === "wall"
        ? object.userData.wallFace as WallFace | undefined ?? nearestWallFace(object.position, size, THREE.MathUtils.radToDeg(object.rotation.y), room)
        : undefined;
      return { surface, wallFace, lockedAxes: mountLockedAxes(surface, wallFace) as Axis3[] };
    };
    const snapObjectToMount = (object: THREE.Group, preferredWallFace?: WallFace) => {
      const { surface, wallFace } = mountInfoFor(object);
      const constrained = constrainPositionToMount({
        surface,
        position: { x: object.position.x, y: object.position.y, z: object.position.z },
        size: object.userData.parametricSize as Point3,
        rotationY: THREE.MathUtils.radToDeg(object.rotation.y),
        room,
        wallFace: preferredWallFace ?? wallFace,
      });
      object.position.set(constrained.position.x, constrained.position.y, constrained.position.z);
      if (constrained.wallFace) object.userData.wallFace = constrained.wallFace;
      return { surface, wallFace: constrained.wallFace, lockedAxes: mountLockedAxes(surface, constrained.wallFace) as Axis3[] };
    };
    ...
    const refreshMeasurements = () => {
      ...
      const mount = mountInfoFor(current);
      setMetrics({ ...spatial, position: { x: current.position.x, y: current.position.y, z: current.position.z }, rotationY: THREE.MathUtils.radToDeg(current.rotation.y), limits: definition ? { min: definition.minSize, max: definition.maxSize } : undefined, issues, mount: { surface: mount.surface, label: MOUNT_SURFACE_LABELS[mount.surface], lockedAxes: mount.lockedAxes } }); scheduleRender(false);
    };
    ...
    const configureTransformLimits = (object: THREE.Group) => {
      ...
      const mount = snapObjectToMount(object, object.userData.wallFace as WallFace | undefined);
      ...
      mount.lockedAxes.forEach(axis => { dragLimits!.min[axis] = origin[axis]; dragLimits!.max[axis] = origin[axis]; });
      ...
      transform.showX = !mount.lockedAxes.includes("x");
      transform.showY = !mount.lockedAxes.includes("y");
      transform.showZ = !mount.lockedAxes.includes("z");
    };
    ...
    const mutateCurrent = (mutation: (object: THREE.Group) => void) => {
      ...
      const wallFace = current.userData.wallFace as WallFace | undefined;
      mutation(current);
      snapObjectToMount(current, wallFace);
    ...
    transform.addEventListener("objectChange", () => {
      if (current) {
        ...
        snapObjectToMount(current, current.userData.wallFace as WallFace | undefined);
        updateBounds(current);
      }
    ...
    const api: SceneController = {
      add: (kind, label) => {
        const asset = getParametricAsset(kind);
        if (!asset) return;
        const mounted = defaultMountedPosition({ surface: asset.defaultMount.surface, size: asset.defaultSize, room, elevation: asset.defaultMount.elevation, offset: 0.25 + (serial % 3) * 0.35 });
        const object = makeObject(kind, label, [mounted.position.x, mounted.position.y, mounted.position.z]);
        if (mounted.wallFace) object.userData.wallFace = mounted.wallFace;
      ...
      duplicate: () => {
        ...
        object.userData.wallFace = source.userData.wallFace;
        snapObjectToMount(object, source.userData.wallFace as WallFace | undefined);
        recordHistory(null, definitionOf(object));
        select(object);
        setObjectCount(selectable.size);
        syncSceneObjects();
  ...
  const interactionGuide = tool === "translate" && hasSelection
    ? { label: isTransforming ? "移动中" : "移动对象", pointerHint: `拖动可用手柄 · ${metrics?.mount.label ?? "默认面"}吸附 · Esc 取消`, touchHint: "拖动手柄移动 · 双指调整视角" }
  ...
  return <main className={`app-shell ${catalogueOpen ? "" : "catalogue-closed"} ${hasSelection ? inspectorOpen ? "" : "inspector-collapsed" : "inspector-hidden"} ${focusMode ? "focus-mode" : ""}`} data-color-mode={colorMode}>
    ...
    {hasSelection && <section className={`inspector ${inspectorOpen ? "open" : "collapsed"}`} aria-label="选中对象属性"><button className="inspector-toggle" onClick={toggleInspector} aria-expanded={inspectorOpen} aria-label="展开或收起属性栏">{inspectorOpen ? "›" : "‹"}</button>{inspectorOpen && <div className="inspector-content"><div className="selection-title"><span>当前选择 · 参数化标模</span><b>{selectedObject?.label}</b><small>输入数值后按 Enter 或点击外部应用</small></div>{metrics ? <>
      <div className={`mount-status mount-${metrics.mount.surface}`}><b>{metrics.mount.label}吸附</b><span>拖拽仅沿吸附面移动，法向坐标已锁定</span></div>
      {metrics.issues.length > 0 && <div className="spatial-warning" role="alert"><b>布局需要检查</b>{metrics.issues.map((issue, index) => <span key={`${issue}-${index}`}>{issue}</span>)}</div>}
      <div className="inspector-section"><h3>位置与旋转</h3><div className="metric-grid position-grid">{AXES.map(axis => <div className="metric-cell" key={axis}><span>{axis.toUpperCase()}</span><MetricInput ariaLabel={`${axis.toUpperCase()} 位置`} disabled={metrics.mount.lockedAxes.includes(axis)} value={metrics.position[axis]} onCommit={value => controller.current?.setPosition(axis, value)} /></div>)}<div className="metric-cell"><span>旋转 Y</span><MetricInput ariaLabel="Y 轴旋转" unit="°" step={1} value={metrics.rotationY} onCommit={value => controller.current?.setRotation(value)} /></div></div></div>
      ...
      <div className="inspector-section clearance-section"><div className="section-heading"><h3>最近空间距离</h3><span>参照墙体或物体</span></div><div className="clearance-grid">{metrics.clearances.map(clearance => <div className={`clearance-cell axis-${clearance.axis}`} key={clearance.key}><div><b>{directionLabels[clearance.key]}</b><small>{clearance.referenceLabel}</small></div><MetricInput ariaLabel={`${directionLabels[clearance.key]}到${clearance.referenceLabel}的距离`} disabled={metrics.mount.lockedAxes.includes(clearance.axis)} min={0} value={clearance.distance} onCommit={value => controller.current?.setClearance(clearance, value)} /></div>)}</div></div>
