...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const makeObject = (kind: string, label: string, position: [number, number, number]) => {
      ...
      if (kind === "bathtub") {
        ...
        bowl.rotation.x = Math.PI / 2; bowl.scale.set(1, 0.55, 1); bowl.position.set(0, 0.35, 0.22); bowl.castShadow = true; group.add(bowl);
        ...
        for (let i = 0; i < 8; i++) { const leaf = new THREE.Mesh(leafGeometry, materials.plant); leaf.scale.set(0.55, 1.7, 0.35); leaf.position.set(Math.cos(i) * 0.2, 0.58 + (i % 3) * 0.1, Math.sin(i) * 0.2); leaf.rotation.z = (i - 4) * 0.12; leaf.castShadow = true; group.add(leaf); }
      }
    ...
    floor.rotation.x = -Math.PI / 2; floor.receiveShadow = true; layout.add(floor);
    ...
    const drawMeasurements = (object: THREE.Group, box3: THREE.Box3, spatial: SpatialMetrics) => {
      ...
      const colors: Record<Axis3, string> = { x: "#244be5", y: "#17a274", z: "#dd7837" };
    ...
    const api: SceneController = {
      ...
      reset: () => { if (activeCamera === orthographicCamera) { orthographicCamera.position.set(0, 10, 0); orthographicCamera.zoom = 1; orthographicCamera.updateProjectionMatrix(); controls.target.set(0, 0, 0); } else { perspectiveCamera.position.set(6.8, 5.7, 7.6); controls.target.set(0, 1, 0); } controls.update(); scheduleRender(false); },
    };
  ...
  return <main className="app-shell">
    ...
    <section className="workspace" aria-label="3D 室内设计画布"><div ref={canvasHost} className="canvas-host" /><div className="view-switch" role="group" aria-label="视图模式"><button className={view === "2D" ? "active" : ""} onClick={() => changeView("2D")}>2D 平面</button><button className={view === "3D" ? "active" : ""} onClick={() => changeView("3D")}>3D 空间</button></div><button className="reset-view" onClick={() => controller.current?.reset()} aria-label="重置视角">⌂</button><div className="room-meta"><span>主卫</span><b>18.6 m²</b><small>{objectCount} 个对象 · 3.05 m 层高</small></div><div className="performance-pill" title={`最近一帧 CPU 提交 ${renderStats.frameMs.toFixed(1)} ms`}><span /> 按需渲染 · {renderStats.calls} calls · {renderStats.triangles.toLocaleString()} tris</div></section>
    ...
    <section className="inspector" aria-label="对象尺寸与空间关系"><div className="selection-title"><span>当前选择</span><b>{selected}</b><small>输入数值后按 Enter 或点击外部应用</small></div>{metrics ? <>
